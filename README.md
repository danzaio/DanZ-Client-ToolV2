...
*   **Lobby Manager**:
    ...
    *   Quickly "Find Match" or "Dodge Queue" with a single click.
*   **Champion Select Automation**:
    *   **Instalock**: Automatically pick your desired champion (or Random). Includes adjustable **Delay**.
    *   **Auto Ban**: Automatically ban a specific champion or choose "None". Includes adjustable **Delay**.
    *   **Backup Pick**: Select a secondary champion if your main is banned or taken.
    *   **Smart Dodge**: Option to auto-dodge if your champion is banned/picked.
*   **Automation Tools**:
    *   **Auto Accept**: Automatically accept the Match Found popup.
    *   **Instant Mute All**: Mute all players immediately upon entering the lobby.
    *   **Reveal Side**: See if you are on **Blue Side** or **Red Side** during champion select.
    *   **Instant Message**: Send a custom chat message (e.g., "Top pref") instantly when joining a lobby.
    *   
...
*   **Identity Manager**:
    ...
    *   **Profile Icon**: Browse, search, and set any profile icon.
    *   **Profile Background**: Set your background to ANY champion and skin.
    *   **Chat Control**: Disconnect or reconnect to the chat server at will.
*   **Profile Spoofer (Visual Only)**:
    *   **Rank Spoof**: Change your visible Rank, Tier, Division, and LP for any queue.
    *   **Stats Spoof**: Set custom Mastery Score, Challenge Crystal Rank, and Challenge Points.
*   **Challenge Extras**:
    *   **Invisible Banner**: Remove your challenge banner.
    *   **Clone Badge**: Clone your first challenge badge to all 3 slots.
...
*   **Mass Disenchant**:
    ...
    *   **Select All** or manually select specific items.
...
*   **Social**:
    ...
    *   **Group Cleaner**: Remove all friends from a specific folder/group.
*   **Loot Utilities**:
    *   **Category Disenchant**: Mass disenchant specific categories (Emotes, Wards, Icons, Eternals).
    *   **Refund**: Refund your last refundable purchase locally.
*   **Utilities**:
...
*   **房间管理**:
    ...
    *   一键“寻找对局”或“秒退队列”。
*   **英雄选择自动化**:
    *   **秒选 (Instalock)**: 自动选择您想要的英雄（或随机）。包含可调节的**延迟**。
    *   **自动禁用 (Auto Ban)**: 自动禁用指定英雄或选择“无”。包含可调节的**延迟**。
    *   **备选英雄**: 如果首选英雄被禁或被抢，自动选择备用英雄。
    *   **智能秒退**: 如果英雄不可用，可选择自动秒退。
*   **自动化工具**:
    *   **自动接受**: 自动接受对局匹配弹窗。
    *   **瞬间全员静音**: 进入房间后立即屏蔽所有玩家。
...
*   **身份管理**:
    ...
    *   **聊天控制**: 随意断开或重新连接聊天服务器。
*   **资料伪装 (仅本地可见)**:
    *   **段位伪装**: 修改任意队列的显示段位、分段和胜点。
    *   **数据伪装**: 设置自定义成就分数、挑战水晶等级和挑战点数。
*   **挑战额外功能**:
    *   **隐形横幅**: 移除您的挑战横幅。
...
*   **Gerenciador de Lobby**:
    ...
    *   "Encontrar Partida" ou "Sair da Fila" rapidamente com um clique.
*   **Automação de Seleção de Campeões**:
    *   **Instalock**: Selecione automaticamente seu campeão desejado (ou Aleatório). Inclui **Delay** ajustável.
    *   **Auto Ban**: Bana automaticamente um campeão específico ou escolha "Nenhum". Inclui **Delay** ajustável.
    *   **Pick Reserva**: Selecione um campeão secundário se o principal for banido ou escolhido.
    *   **Smart Dodge**: Opção de quitar automaticamente se seu campeão for banido/escolhido.
*   **Ferramentas de Automação**:
    *   **Auto Aceitar**: Aceita automaticamente o pop-up de Partida Encontrada.
    *   **Mute All Instantâneo**: Muta todos os jogadores imediatamente ao entrar no lobby.
...
*   **Gerenciador de Identidade**:
    ...
    *   **Ícone de Perfil**: Navegue, pesquise e defina qualquer ícone de perfil.
    *   **Fundo do Perfil**: Defina seu fundo com QUALQUER campeão e skin.
    *   **Controle de Chat**: Desconecte ou reconecte ao servidor de chat à vontade.
*   **Spoofer de Perfil (Apenas Visual)**:
    *   **Rank Spoof**: Altere seu Elo, Divisão e PDL visíveis para qualquer fila.
    *   **Stats Spoof**: Defina Pontuação de Maestria, Rank Cristal e Pontos de Desafio personalizados.
*   **Extras de Desafio**:
    *   **Banner Invisível**: Remova seu banner de desafio.
...
*   **Social**:
    ...
    *   **Deletar Todos** os pedidos de amizade.
    *   **Limpador de Grupo**: Remova todos os amigos de uma pasta/grupo específico.
*   **Utilitários de Espólios**:
    *   **Desencantar por Categoria**: Desencante em massa categorias específicas (Emotes, Wards, Ícones, Eternos).
    *   **Reembolso**: Reembolse sua última compra reembolsável localmente.
*   **Utilitários**:
...
*   **Lobi Yöneticisi**:
    ...
    *   Tek tıklamayla hızlıca "Maç Bul" veya "Sıradan Çık".
*   **Şampiyon Seçimi Otomasyonu**:
    *   **Instalock**: İstediğiniz şampiyonu (veya Rastgele) otomatik seçin. Ayarlanabilir **Gecikme** içerir.
    *   **Otomatik Ban**: Belirli bir şampiyonu otomatik yasaklayın veya "Hiçbiri"ni seçin. Ayarlanabilir **Gecikme** içerir.
    *   **Yedek Seçim**: Ana şampiyonunuz yasaklanırsa veya alınırsa ikincil bir şampiyon seçin.
    *   **Akıllı Çıkış (Dodge)**: Şampiyonunuz yasaklanırsa/alınırsa otomatik çıkma seçeneği.
*   **Otomasyon Araçları**:
    *   **Otomatik Kabul**: Maç Bulundu uyarısını otomatik olarak kabul et.
    *   **Anında Herkesi Sustur**: Lobiye girer girmez tüm oyuncuları sustur.
...
*   **Kimlik Yöneticisi**:
    ...
    *   **Profil Simgesi**: Herhangi bir profil simgesine göz atın, arayın ve ayarlayın.
    *   **Profil Arka Planı**: Arka planınızı HERHANGİ bir şampiyon ve kostüm yapın.
    *   **Sohbet Kontrolü**: Sohbet sunucusuna istediğiniz zaman bağlanın veya bağlantıyı kesin.
*   **Profil Spoofer (Sadece Görsel)**:
    *   **Rütbe Spoofer**: Herhangi bir sıra için görünür Rütbe, Kademe, Bölüm ve LP'nizi değiştirin.
    *   **İstatistik Spoofer**: Özel Ustalık Puanı, Kristal Rütbesi ve Meydan Okuma Puanları ayarlayın.
*   **Meydan Okuma Ekstraları**:
    *   **Görünmez Banner**: Meydan okuma afişinizi kaldırın.
...
*   **Sosyal**:
    ...
    *   **Tümünü Sil**: Tüm arkadaşlık isteklerini sil.
    *   **Grup Temizleyici**: Belirli bir klasörden/gruptan tüm arkadaşları kaldırın.
*   **Ganimet Araçları**:
    *   **Kategori Ayrıştırma**: Belirli kategorileri (İfadeler, Totemler, Simgeler, Ebediler) toplu ayrıştırın.
    *   **İade**: Son iade edilebilir satın alma işleminizi yerel olarak iade edin.
*   **Araçlar**:
...
*   **Lobby-Manager**:
    ...
    *   Schnelles "Match finden" oder "Warteschlange verlassen" mit einem Klick.
*   **Champion-Auswahl-Automatisierung**:
    *   **Instalock**: Wählen Sie automatisch Ihren gewünschten Champion (oder Zufällig). Inklusive einstellbarer **Verzögerung**.
    *   **Auto Ban**: Bannen Sie automatisch einen bestimmten Champion oder wählen Sie "Keinen". Inklusive einstellbarer **Verzögerung**.
    *   **Backup Pick**: Wählen Sie einen sekundären Champion, wenn Ihr Hauptchampion gebannt oder genommen wurde.
    *   **Smart Dodge**: Option zum automatischen Verlassen, wenn Ihr Champion gebannt/genommen wurde.
*   **Automatisierungstools**:
    *   **Auto Accept**: Akzeptieren Sie automatisch das "Match gefunden"-Popup.
    *   **Sofort alle stummschalten**: Schalten Sie alle Spieler sofort nach Betreten der Lobby stumm.
...
*   **Identitätsmanager**:
    ...
    *   **Profilsymbol**: Durchsuchen, suchen und setzen Sie ein beliebiges Profilsymbol.
    *   **Profilhintergrund**: Setzen Sie Ihren Hintergrund auf JEDEN Champion und Skin.
    *   **Chat-Steuerung**: Trennen oder verbinden Sie sich nach Belieben mit dem Chat-Server.
*   **Profil-Spoofer (Nur visuell)**:
    *   **Rang-Spoof**: Ändern Sie Ihren sichtbaren Rang, Stufe, Division und LP für jede Warteschlange.
    *   **Statistik-Spoof**: Setzen Sie benutzerdefinierte Meisterschaftspunkte, Kristallränge und Herausforderungspunkte.
*   **Herausforderungs-Extras**:
    *   **Unsichtbares Banner**: Entfernen Sie Ihr Herausforderungsbanner.
...
*   **Soziales**:
    ...
    *   **Alle löschen**: Alle Freundschaftsanfragen löschen.
    *   **Gruppenbereiniger**: Entfernen Sie alle Freunde aus einem bestimmten Ordner/einer bestimmten Gruppe.
*   **Beute-Dienstprogramme**:
    *   **Kategorie-Entzauberung**: Massenhaftes Entzaubern bestimmter Kategorien (Emotes, Wards, Symbole, Ewige).
    *   **Rückerstattung**: Erstatten Sie Ihren letzten erstattungsfähigen Kauf lokal.
*   **Dienstprogramme**:
